...
function questionSelection(parsedQuestions, test ,numeroQuestion, action) {
    if (action === 'selection') {
        ...
        test.push(parsedQuestions[numeroQuestion - 0]);
        ...
        // Retirer la question de la liste des questions sélectionnées
                if (!test.includes(parsedQuestions[numeroQuestion - 0])) {
                    console.log("On ne peut supprimer un element qui n'existe pas".red);
                } else if(test.includes(parsedQuestions[numeroQuestion - 0])){
                    
                // Vous pourriez avoir une propriété pour stocker les questions sélectionnées dans votre classe
                test = test.filter((question) => question !== parsedQuestions[numeroQuestion]);
